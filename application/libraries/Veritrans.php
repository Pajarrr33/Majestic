	...
	public static function status($id)
 	{
   	...
   	*/
	   ...
	   {
		   $url = Veritrans::getBaseUrl() . '/' . $id . '/cancel';
		   $serverKey = Veritrans::$serverKey;
	   
		   $response = Veritrans::post($url, $serverKey, false);
	   
		   if ($response->status_code == 200) {
			   return "Transaction canceled successfully.";
		   } else {
			   // Log or display the error response for debugging
			   // You can access $response->body to get the full response content
			   // You can access $response->status_code to get the HTTP status code
			   return "Transaction cancellation failed. Status code: " . $response->status_code;
		   }
	   }
